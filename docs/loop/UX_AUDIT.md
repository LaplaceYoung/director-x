...
- A selected candidate now enters the public `directorx_build_rough_cut` boundary. `inspect` reports whether an editor session is available; `propose` creates a reversible evidence draft without committing the timeline.

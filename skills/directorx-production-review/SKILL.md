...
18. For `reference-replication`, run `directorx_score_reference_replication` only after `frame_audit_report.json` and `final_review.json` are complete. Compare source video/audio and output using time-aligned evidence, score structure, pacing, camera, motion, audio, and originality, and route the result to `pass_export`, `needs_edit`, or `regenerate`. Never export solely because the file is playable.

...
9. Classify failures causally: prompt ambiguity, reference conflict, unsupported capability, provider parameter error, identity/geometry drift, composition, motion/physics, boundary mismatch, text, policy/rights, or provider rejection. After `directorx_review_generation_candidate` returns anything other than accept, call `directorx_compile_generation_repair`. Execute only its single `controlVariable`; do not combine its patch with a hand-written second change.
...
12. Before another paid draw, honor the compiled repair disposition: deterministic text/UI goes to editing, missing references go back to grounding, boundary/action overload goes back to shot design, provider/capability failures require rerouting, and exhausted attempts or budget require a native decision instead of a silent retry.

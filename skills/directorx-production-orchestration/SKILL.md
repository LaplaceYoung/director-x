...
2. Diagnose ambiguity. Persist an `intake` interaction, ask no more than three strategy-changing questions through its exact Codex `request_user_input` host action, resolve the same request ID, and pass it to `directorx_confirm_intake`. Then translate the resolved intent into director language and call the intent-resolution and Director-document tools before classifying the production mode. Platform, production route, and asset readiness require brief or native user-answer evidence.
...
5. Select the matching built-in pipeline with `directorx_select_pipeline`; treat its eight stages as the minimum manifest. Extend outputs and reviewers for the production mode without deleting core gates.

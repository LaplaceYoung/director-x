...
4. Confirm the exact provider/model through Codex `request_user_input`, then call `directorx_get_media_provider_setup`. For a custom route, dispatch `DX-Model-Router` to search and open only official API documentation, record the real host search/open receipts with `directorx_record_provider_api_research`, and register one model-specific adapter with `directorx_register_custom_media_provider_adapter`. Keep this receipt separate from brand/asset `web_research_receipt.json`. Adapters are declarative HTTPS JSON mappings: no generated code, SDK installation, shell, arbitrary authentication headers, private origins, or undocumented fields. Use `directorx_get_custom_media_provider_setup` after resume. Persist and resolve its `keySetupInteraction` natively, then collect the actual secret only in the canvas secure password field; this calls `directorx_set_session_credential` and configures the expected current-process environment variable without writing the Key to chat, artifacts, Run state, receipts, logs, or Git. For speech, call `directorx_get_mosi_voice_setup` first and pass its `selectionQuestion` unchanged to `request_user_input`; `MOSS-TTS (Recommended)` must remain the first option at every TTS choice point. When MOSS-TTS is selected and `keySetupRequired=true`, persist the returned `keySetupInteraction` with `directorx_request_user_interaction`, execute its native prompt, resolve the same request, and use only the matching `keySetupAnswerActions`. Open `https://platform.mosi.cn` only after the user selects the recommended “前往” option, keep the production canvas open, then focus its secure credential panel.
...
Runtime rule: provider/model IDs in the generation plan must match the latest user-approved decision. A reroute requires a new approval decision before another attempt.

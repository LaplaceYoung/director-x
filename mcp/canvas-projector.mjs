...
const APPROVAL_INSTRUCTIONS = {
  ...
  music_strategy: "请回到 Codex 先选择正版曲库、本地音乐、音乐模型或不单独配乐",
  music_asset_selection: "音乐检索和审计完成后，请回到 Codex 选择具体曲目",
...
export function projectCanvas(snapshot) {
  ...
  const evidenceQueryNodes = Object.values(snapshot.videoEvidenceQueries ?? {}).map((query, offset) => ({ id: `evidence-query:${query.plan.queryId}`, type: "decision", stage: "research", label: query.plan.question, detail: `${query.status} · ${query.trace?.stopReason ?? `${query.searches?.length ?? 0} 次检索`}`, status: query.status === "complete" ? "complete" : query.status === "input_required" ? "blocked" : query.status === "stopped" ? "failed" : "active", x: 500 + offset * 285, y: 1480, artifactRef: query.evidenceBundle ? "evidence_bundle.json" : query.trace ? "retrieval_trace.json" : query.searches?.length ? `video_search_results.${query.searches.at(-1).searchId}.json` : "video_query_plan.json", metadata: { plan: query.plan, searches: query.searches ?? [], trace: query.trace, evidenceBundle: query.evidenceBundle } }));
  ...
  const evidenceRail = { queries: Object.values(snapshot.videoEvidenceQueries ?? {}).map((query) => {
    ...
    const clipsByNode = new Map((snapshot.evidenceClips ?? []).filter((clip) => clip.queryId === query.plan.queryId).map((clip) => [clip.nodeId, clip]));
    const selectedHits = (query.trace?.selectedNodeIds ?? []).map((id) => ({ ...byId.get(id), nodeId: id })).filter((item) => item.range).map((item) => ({ ...item, targetNodeId: `asset:${index.source.assetId}`, targetArtifactRef: index.source.artifactRef ?? null, clipArtifactRef: clipsByNode.get(item.nodeId)?.artifactRef ?? null, coordinateSpace: "source", startSeconds: evidenceSeconds(item.range.start), durationSeconds: evidenceSeconds(item.range.duration) }));
    const candidateHits = (query.searches ?? []).at(-1)?.result?.candidates ?? [];
    return { queryId: query.plan.queryId, question: query.plan.question, status: query.status, stopReason: query.trace?.stopReason ?? null, coverage: query.evidenceBundle?.coverage ?? null, claim: query.evidenceBundle?.claim ?? null, conflicts: query.trace?.conflicts ?? [], hits: selectedHits, candidateHits: selectedHits.length ? [] : candidateHits.map((item) => ({ ...item, targetNodeId: `asset:${index.source.assetId}`, targetArtifactRef: index.source.artifactRef ?? null, clipArtifactRef: clipsByNode.get(item.nodeId)?.artifactRef ?? null, coordinateSpace: "source" })) };
  }) };
...
function buildWorkflowProjection({ goalNode, approvals, stageNodes, runModeNode, budgetNode, completionNode, subagents, executionNodes }) {
  ...
  const approvalStage = (kind) => kind === "delivery" ? "delivery" : ["image_model", "video_model", "voice_model", "music_strategy", "music_asset_selection", "music_route", "model", "provider"].includes(kind) ? "generation" : "intake";

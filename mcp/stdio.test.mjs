...
test("advertises the mandatory side-Browser startup contract during MCP initialization", async () => {
  ...
  try {
    ...
    assert.match(initialized.instructions, /in-app Browser/);
    assert.match(initialized.instructions, /visibility to true/);
    assert.match(initialized.instructions, /Storyboard limited to generated Markdown and essential real image\/video\/audio media/);
    assert.match(initialized.instructions, /browser\.tabs\.finalize/);
    assert.match(initialized.instructions, /never downgrade to chat text/);
    assert.match(initialized.instructions, /directorx_plan_production_team/);
    assert.match(initialized.instructions, /directorx_plan_parallel_subagents/);
    assert.match(initialized.instructions, /builtin_compatibility/);
    assert.match(initialized.instructions, /collaboration_task/);
    assert.match(initialized.instructions, /task_name\/fork_turns\/message/);
    assert.match(initialized.instructions, /sub_agent_activity\.agent_thread_id/);
    assert.match(initialized.instructions, /afterSpawn/);
    assert.match(initialized.instructions, /afterAnswer/);
    assert.match(initialized.instructions, /directorx_compile_claim_proof_map/);
    assert.match(initialized.instructions, /directorx_compile_shot_grounding_plan/);
    assert.match(initialized.instructions, /directorx_finalize_shot_grounding/);
    assert.match(initialized.instructions, /directorx_compile_visual_prompt_pack/);
    assert.match(initialized.instructions, /directorx_compile_scene_coverage_plan/);
    assert.match(initialized.instructions, /directorx_record_scene_coverage_review/);
    assert.match(initialized.instructions, /directorx_query_cinematic_references/);
    assert.match(initialized.instructions, /depth 1\/1/);
    assert.match(initialized.instructions, /directorx_review_shot_sequence/);
    assert.match(initialized.instructions, /without inventing close_agent/);
    assert.match(initialized.instructions, /directorx_acquire_web_image_asset/);
    assert.match(initialized.instructions, /actually execute host web search and source-open actions/);
    assert.match(initialized.instructions, /first\/end-frame chain/);
    assert.match(initialized.instructions, /all-decoded-frame extraction/);
    assert.match(initialized.instructions, /directorx_compile_reference_replication_plan/);
    assert.match(initialized.instructions, /platform\.mosi\.cn/);
    assert.match(initialized.instructions, /music_route/);
    assert.match(initialized.instructions, /audio_responsibility_plan\.json/);
    assert.match(initialized.instructions, /official pricing page/);
    assert.match(initialized.instructions, /post_production_edit/);
    assert.match(initialized.instructions, /directorx_start_opencut_editor/);
    assert.match(initialized.instructions, /directorx_propose_evidence_rough_cut/);
    assert.match(initialized.instructions, /DX-Quality-Reviewer/);
    assert.match(initialized.instructions, /detector signal is only a review candidate/);
    assert.match(initialized.instructions, /directorx_record_final_review_evidence/);
...
test("routes the first preflight to a standalone browser canvas", async () => {
  ...
  try {
    ...
    assert.deepEqual(message.result.structuredContent.requiredApprovals, ["goal_entry", "production_budget", "image_provider_and_model", "video_provider_and_model", "voice_provider_model_and_voice", "background_music_route"]);
...
test("persists, deduplicates, and enforces native request_user_input gates", async () => {
  ...
  try {
    ...
    const first = (await send(203, "directorx_request_user_interaction", interactionInput)).result.structuredContent;
    const duplicate = (await send(204, "directorx_request_user_interaction", interactionInput)).result.structuredContent;
